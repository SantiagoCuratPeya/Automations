...
sheet = Spread(sheet_id, wks_name, config=cred_sheets)
...
wks_name = 'Madurez'
sheet = Spread(sheet_id, wks_name, config=cred_sheets)
madurez = sheet.sheet_to_df(index=0, header_rows=1)
...
# Mando a mayusculas los filtros
madurez[madurez.columns[0]] = madurez[madurez.columns[0]].str.upper()
madurez[madurez.columns[1]] = madurez[madurez.columns[1]].str.upper()
dic_mad = {}
for i in madurez['Madurez'].unique().tolist():
    dic_mad[i] = madurez[madurez['Madurez'] == i]['Country'].tolist()
# Defino la funcion de madurez
def func_madurez(cn):
    '''
        Esta función recibe un parámetro:
        . El Nombre del País
        En caso de errores:
        . Devuelve "No Clasificable"
    '''
